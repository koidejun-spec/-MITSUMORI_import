    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-1">売価計算方式</p>
            <p className="text-xs text-slate-400 mb-3">原価から売価を算出する方法を選択</p>
              ...
              <div className="flex items-center gap-2">
                {isMarkup ? (
                  <>
                    ...
                    />
                    <span className="text-xs text-slate-400">例）1.2 → 原価の20%増し</span>
                  </>
                ) : (
                  <>
                    <input
                      ...
                      className="w-24 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-teal-500"
                    />
                    <span className="text-xs text-slate-400">%（例：20 → 粗利率20%）</span>
                  </>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 text-xs border border-slate-200 rounded-lg px-2 py-2 focus:outline-none focus:ring-1 focus:ring-teal-500"
                />
                <input
                  type="number"
                  ...
                  className="w-20 text-xs border border-slate-200 rounded-lg px-2 py-2 focus:outline-none focus:ring-1 focus:ring-teal-500"
                />
                <button
                  onClick={addCategoryRate}
                  className="text-xs bg-teal-600 text-white px-3 py-2 rounded-lg hover:bg-teal-700"
                >追加</button>
              </div>
            </div>
          </div>
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-1">粗利計算基準</p>
            <p className="text-xs text-slate-400 mb-3">粗利率の算出に使用する売価の基準</p>
            <select
              value={profitBase}
          ...
          <div>
            <p className="text-sm font-semibold text-slate-700 mb-1">端数処理ルール</p>
            <p className="text-xs text-slate-400 mb-3">単価を逆算する際（金額÷数量）の端数処理方法</p>
            <select
              value={roundingMode}
        ...
        <div className="px-6 py-4 border-t border-slate-100">
          <button
            onClick={onClose}
            className="w-full bg-slate-800 hover:bg-slate-900 text-white font-semibold py-2.5 rounded-lg transition-colors text-sm"
          >
            閉じる
          </button>
        </div>
